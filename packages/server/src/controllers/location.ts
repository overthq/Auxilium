...
export const getRoute: RequestHandler = async (req, res) => {
	...
	try {
		const response = await fetch(
			`${MAPBOX_BASE_URL}/mapbox/walking/${fromLongitude},${fromLatitude};${toLongitude},${toLatitude}.json?access_token=${env.MAPBOX_ACCESS_TOKEN}&geometries=geojson`
		);
...
export const getAddress: RequestHandler = async (req, res) => {
	...
	try {
		const response = await fetch(
			`${MAPBOX_BASE_URL}/mapbox.places/${longitude},${latitude}.json?access_token=${env.MAPBOX_ACCESS_TOKEN}`
		);
